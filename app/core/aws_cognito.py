...
class AWSCognito:
    ...
    def user_signup(self, user: UserSignup):
        secret_hash = base64.b64encode(hmac.new(bytes(AWS_COGNITO_APP_CLIENT_SECRET, 'utf-8'), bytes(user.username + AWS_COGNITO_APP_CLIENT_ID, 'utf-8'), digestmod=hashlib.sha256).digest()).decode()
    ...
    def verify_account(self, data: UserVerify):
        secret_hash = base64.b64encode(hmac.new(bytes(AWS_COGNITO_APP_CLIENT_SECRET, 'utf-8'), bytes(data.username + AWS_COGNITO_APP_CLIENT_ID, 'utf-8'), digestmod=hashlib.sha256).digest()).decode()
    ...
    def resend_confirmation_code(self, username: str):
        ...
        secret_hash = base64.b64encode(hmac.new(bytes(AWS_COGNITO_APP_CLIENT_SECRET, 'utf-8'), bytes(username + AWS_COGNITO_APP_CLIENT_ID, 'utf-8'), digestmod=hashlib.sha256).digest()).decode()
